...
- Two pools exist: the **global pool** at `~/.claude/skills/` (symlinked from `claude-rails/global/skills/` by sync) and the **project-local pool** at `<project>/.claude/skills/`. Both are visible when Claude Code runs inside a project; the project pool shadows the global pool by skill name.
