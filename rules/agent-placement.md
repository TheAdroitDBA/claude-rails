...
- Two pools exist: the **global pool** at `~/.claude/agents/` (symlinked from `claude-rails/global/agents/` by sync) and the **project-local pool** at `<project>/.claude/agents/`. Both are loaded when Claude Code runs inside a project; the project pool shadows the global pool by agent name.
...
- **Promotion (project -> global):** when an agent's content becomes fully portable (all project references stripped), move it to `claude-rails/global/agents/` via `git mv`. Commit both sides.
