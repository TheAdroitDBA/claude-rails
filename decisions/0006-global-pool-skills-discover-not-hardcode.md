...
Decision 0004 established the two-pool placement model: global-pool artifacts must be project-neutral. The rule "be project-neutral" is clear, but the operational question -- "what do I do instead of hardcoding?" -- was not answered. Without an explicit alternative, skills take shortcuts: embedding absolute paths, project-specific hostnames, or hardcoded tool configurations. The litmus test in 0004 (does it reference a specific project, IP, or path) catches violations at placement time but does not prevent them at authoring time. An operational rule is needed to close the gap between intent and practice.

## Decision

Global-pool skills must discover project context at invocation time rather than hardcoding it. Specifically:

- **Paths are discovered, not declared.** Use `git rev-parse --show-toplevel` for the adopted-repo root and relative paths from that anchor. Never embed an absolute path that names a specific user or project.
...
- **Domain conventions stay with their domain.** When a skill embeds a specific project's API shape, naming scheme, or tooling, that content belongs in that project's `.claude/skills/`, not the global pool. Domain-expert skills contain portable knowledge only; project-specific conventions live in project-pool skills.
- **Exemptions are explicit.** A global-pool skill that genuinely must reference a specific tool or format (e.g. a skill that wraps ffmpeg may name ffmpeg) documents the exemption in its `## Notes` section. The exemption covers tooling, never a project.
...
- **Generate skills per-project with local values baked in.** Rejected. Defeats the purpose of a shared framework: two machines running the same skill would diverge by machine state.
