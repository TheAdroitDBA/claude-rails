...
- `Concern: framework` / `Concern: dogfood` / `Concern: both` is an orthogonal metadata label. It describes blast radius (ships to adopted repos, stays in claude-config, or both). It does NOT determine where the file sits.
...
- The root-level claude-config feature docs are correctly placed when their scopes span multiple directories, even if the list feels long. Ten root-level cross-cutting docs is not sprawl; it is an honest reflection of how many repo-wide concerns the framework governs.
